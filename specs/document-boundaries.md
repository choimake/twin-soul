...
この文書は、`twin-soul` 内で扱う文書と再利用資産の境界を整理する。

## 目的

- external に展開する単位と、`twin-soul` 内部だけで使う文書を分ける
- skill の設計時に、内部文書を前提にしすぎるコンセプトずれを防ぐ
...
- installer の振る舞い、文書構成、配布元としての運用など、リポジトリ内部の仕様を残す
...
新しい知識や文書を追加するときは、次の順で置き場を判断する。

1. 外部プロジェクトへ単体で展開したいか
2. 利用先リポジトリに `twin-soul` 内部文書がなくても成立するか
