...
name: gap-analysis
description: >-
  PRD（または同等の要求一覧）と検証・調査結果を突き合わせ、実現可否と根拠を表形式でまとめる Gap 分析する。Gap 分析表、実現可否、検証結果、PRD 要件の充足確認、スパイク結果の整理時に使う。バックログアイテムの起票そのもの、実装手順や変更ファイル一覧を主とする plan ファイルの作成は対象外。
---
...
PRD 上の要求と、実測・調査で得た証拠を対応づけ、判定凡例に沿った実現可否と制限・代替を 1 ドキュメントに収束させる。他の Agent Skill を前提としない。
