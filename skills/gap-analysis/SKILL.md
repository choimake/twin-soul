...
PRD 上の要求と、実測・調査で得た証拠を対応づけ、判定凡例に沿った実現可否と制限・代替を 1 つの文書にまとめる。他の Agent Skill を前提としない。

## 使う場面

- PRD 要件と検証（スパイク・PoC・試験環境での確認）結果を突き合わせたいとき
- 「可 / 条件付き可 / 不可」を関係者と合意したいとき
...
6. 各要件に到達した証拠レベルを書く。自己申告の文章は根拠にしない。未証明を「可」に丸めない。対応表は knowledge を参照。
7. サマリ表で一覧化し、必要なら「今後の検討事項」を追記する。
8. `write-file` は、保存先パスがユーザーから明示されたときのみそのパスへ書き込む。未指定なら `draft` で本文を返す。

判定の細部・品質チェック・出力モードの差分は [references/gap-analysis-knowledge.md](references/gap-analysis-knowledge.md) を参照する。

## 期待する出力

- Gap 分析表（Markdown）：表紙メタ、判定凡例、任意の共通前提、要件ごとの表（証拠レベルを含む）、サマリ、任意の今後の検討
- `review` 時は構成の妥当性・不足列・証拠の欠落・未証明の可への丸めの指摘
...
- 各要件の判定が証拠レベルと対になり、言っただけでは可にしない
