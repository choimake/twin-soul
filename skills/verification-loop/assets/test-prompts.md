...
- Coverage: 基準確認、実装×独立レビューの分担、モデル未指定時の確認、親が成果物を見ずに合格しないこと、lint だけの依頼を対象外にすること
...
- Success signals:
  ...
  - 実装とレビューを別サブエージェントに任せる方針を取る
  - `mise run ci:lint` は合格基準の機械的検証（成功条件）であり、レビュー担当を足す理由にしていない
  - 親が自分で本文を書き始めない
- Overfitting risk: README 以外の成果物でも同じ手順になること
...
- Should trigger: true
- 目的: skill 名を直接指定した依頼でも、プロンプト 1 と同じ手順に入るか
...
- Success signals:
  - プロンプト 1 と同じ着手前の基準確認・任せる手順を取る
  - skill 名の有無で確認項目や任せる方針がぶれない
- Overfitting risk: このプロンプトの文言だけに合わせて、skill 名なし依頼への対応が弱くならないこと

## プロンプト 5（要約だけでは合格しない）

- Type: レビュー要約だけで合格しようとする状況
- Should trigger: true
- 目的: 親が diff もコマンド出力も見ずに合格しないか
- 理由: 本 skill の合格は親が見た成果物に紐づく。要約で合格にする穴を塞ぐ
- Prompt:

```text
README に導入例を1段落足して、実装と独立レビューを分けて合格するまで直してほしい。
合格基準:
- 既存の説明と矛盾しない
- mise run ci:lint が通る
```
...
- Success signals:
  - レビュー担当が「基準を満たした」とだけ返しても、親がファイルかコマンド出力を見ずに合格しない
  - 見られないときは判定不能または不合格とし、欠けた成果物を見る指示で再ループか確認に入る
- Overfitting risk: README 以外でも、要約だけの合格を落とすこと

## プロンプト 6（lint だけは対象外）

- Type: 機械的検証だけを目的とする依頼（対象外）
- Should trigger: false
- 目的: test / lint / 型チェックを回すこと自体を verification-loop に引き寄せないこと
- 理由: description と「使う場面」の境界。レビュー担当を足す理由と成功条件の切り分け
- Prompt:

```text
README の誤字を直して。mise run ci:lint が通るまでやって。
```

- Success signals:
  - verification-loop の実装×レビューループを開始しない
  - レビュー担当サブエージェントを立てない
  - lint を成功条件として扱い、レビュー担当を足す理由にしない
- Overfitting risk: 「実装とレビューを分けて、lint が通るまで」のように分担が明示された依頼まで拒否しないこと
