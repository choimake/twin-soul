...
- 過適合リスク: 特定言語やフレームワークへの最適化を避け、判断プロセスの質を見る

## プロンプト 1

- Type: 典型ケース — 新規プロジェクトで Core を定義したい
- 目的: 現状解析 → 仮説提示 → トレードオフ質問 → 草案化の基本フローが回ること
- 理由: skill の中核ユースケースが動かなければ初期定義として成立しない
- Prompt:
...
- Success signals:
  ...
  - 構造の違う仮説を 2 案出している（同じ形の言い換えだけではない）
  - 1 案だけで草案へ進んでいない
  - 「Prisma 依存を許容する例外にするか、分離して棚上げするか」のようなトレードオフ質問を返している
  - オープンクエスチョンではなく、事実つきの二択/三択になっている
  - 確認質問に `src/services/orderService.ts` や `src/models/prismaClient.ts` など具体的なファイルパスを含めている
- 過適合リスク: TypeScript / Prisma 固有の知識に寄りすぎると、他言語で使えなくなる
...
- Success signals:
  ...
  - 「README の意図を今から実現するか / 現状を棚上げして段階的に直すか」のトレードオフ質問を返している
  - 構造の違う仮説を 2 案出している
  - `ports/` が空である事実から、ヘキサゴナルの実現度を評価している
  - 確認質問に `domain/models.py` や `domain/services.py` など具体的なファイルパスを含めている
- 過適合リスク: Django 固有の知識に寄りすぎないこと。「ドキュメントと実態の乖離」という汎用的な判断力を見る

## プロンプト 3

- Type: 反証ケース — 理想論を押し通すユーザーへの反論と、棚上げする既存負債の提案
- 目的: 現実と乖離した要求に対し、反証を示して現実的な妥協案（棚上げ）を提案できること
...
- Success signals:
  ...
  - まず棚上げする既存負債を広く取り、`services/` の拡充から始める段階的アプローチを提案している
  - 最終形だけでなく、現実的な第一歩を `.architecture-core.md` 草案に入れている
  - 「最終形を書いてほしい」に対し、現時点の棚上げと将来の目標を分けて提示している
  - いきなり断定せず、まず現状解析の証拠と構造の違う 2 案を出している
  - 確認質問に `app/models/order.rb` や `app/services/payment_service.rb` など具体的なファイルパスを含めている
  - ユーザー合意なしに実装手順へ進んでいない
- 過適合リスク: Rails 固有のパターンに寄りすぎないこと。「理想論 vs 現実」のトレードオフ判断力を見る
