...
- root `apm.yml` に `path: ./skills/...` を列挙すると、remote package として `choimake/twin-soul#main` を導入したときに consumer filesystem への local path dependency と見なされ、APM が拒否する
...
- このリポジトリの root `apm.lock.yaml` は、local path dependency 管理をやめた後は不要とする
- 利用先リポジトリでは引き続き `apm.lock.yaml` をコミットし、resolved commit と展開内容を固定する
...
- remote package から consumer filesystem の local path を参照する構成を避けられる
