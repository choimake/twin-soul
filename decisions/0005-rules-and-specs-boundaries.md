...
`decisions/0001-document-boundaries.md` では、`skills/` を外部展開単位、`specs/` と `decisions/` を `twin-soul` 内部文書として扱う方針を決めた。一方で、`rules/` は位置づけを確定せず、当面は外部展開の主役として扱わない状態にしていた。

その後、APM 移行により `skill` の配布対象は `skills/` に明確化された。`decisions/0004-use-apm-for-shared-skill-distribution.md` でも、旧 `sync.sh` が担っていた `rules/` シンボリックリンクと入口文書生成は APM 配布とは別に整理する必要があると記録している。
...
`twin-soul` では、文書と `rules` の境界を次のように扱う。
...
`rules/` を外部プロジェクトへ配布したくなった場合は、APM 指示文書などの配布単位として別途設計し、`specs/` と `decisions/` を更新する。
...
- Cursor / Claude Code の 実行環境別の `rules` ディレクトリを前提にした運用は使わない
- 将来 `rules` を外部配布したくなった場合は、APM 指示文書などの設計を追加で行う必要がある
